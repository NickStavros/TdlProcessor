...
{ PostInfo "Validating commit message format..."
  # Determine commit message file (from argument, config, or fallback)
  local COMMIT_MSG_FILE="${USER_COMMIT_MSG_FILE:-$(loadJsonValue '.commit_message.file')}"
  if [ -z "$COMMIT_MSG_FILE" ] || [ "$COMMIT_MSG_FILE" = "null" ]; then
  ...
  # Load expected commit message format
  local COMMIT_REGEX=$(loadJsonValue '.commit_message.regex')
  local EXAMPLE_COMMIT=$(loadJsonValue '.commit_message.example')
  # Validate the commit message
  if ! grep -Eiq "$COMMIT_REGEX" "$COMMIT_MSG_FILE"; then
    PostErr "Invalid commit message format!"
    echo "Expected format: <type>(<module>): <summary>"
    echo "Example: $EXAMPLE_COMMIT"
    PostErr "Refer to pre_commit_config.json for details."
    ((VALID_STATE++))
    return 1
  fi
