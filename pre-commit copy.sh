...
{ PostInfo "Starting environment setup..."
  ...
  SPELLCHECK_ERRORS=0            # Number of Spellcheck errors encountered

  # Load JSON values ONCE into memory
  loadJsonConfigValues
  # Security Level
  SECURITY_LEVEL=$(getJsonValueOrDefault "._classification.level" "PUBLIC", "SECURITY LEVEL" )
  # Verbosity Settings
  LOCAL_VERBOSE_DEBUG=$(getJsonValueOrDefault ".pre_commit.verbose.debug.enabled" "false" "DEBUG")
  LOCAL_VERBOSE_INFO=$(getJsonValueOrDefault  ".pre_commit.verbose.info.enabled" "false"  "INFO")
  LOCAL_VERBOSE_WARN=$(getJsonValueOrDefault  ".pre_commit.verbose.warn.enabled" "false"  "WARN")
  LOCAL_VERBOSE_ERROR=$(getJsonValueOrDefault ".pre_commit.verbose.error.enabled" "false" "ERROR")
  # Resolve scan path from loaded JSON
  SCAN_PATH=$(getJsonValue ".scan_scope.default_path")
  if [[ -z "$SCAN_PATH" ]]; then
    SCAN_PATH="$DIST_DIR"             # Default SCAN_PATH
  fi
...
PostInfo()
{ if [[ "${LOCAL_VERBOSE_INFO:-false}" == "true" ]]; then 
    echo "--   [INFO] $(timestamp) -> $1" 
  fi
} # End function PostInfo
PostDebug()
{ if [[ "${LOCAL_VERBOSE_DEBUG:-false}" == "true" ]]; then 
    echo "--=== [DEBUG] $(timestamp) -> $1"
  fi
} # End function PostDebug
PostWarn()
{ if [[ "${LOCAL_VERBOSE_WARN:-false}" == "true" ]]; then 
    echo "--+++ [WARN] $(timestamp) -> $1"
  fi
} # End function PostWarn
PostErr()
{ if [[ "${LOCAL_VERBOSE_ERROR:-false}" == "true" ]]; then 
...
getJsonValue() {
  local KEY="$1"
  local VALUE
  VALUE=$(jq -r "$KEY // empty" <<< "$JSON_CONFIG" 2>/dev/null)
  if [[ "$VALUE" == "null" || -z "$VALUE" || "$VALUE" == "empty" ]]; then
...
# ---------- Function: Get JSON Value from Loaded Config or use default ----------
getJsonValueOrDefault() {
  local key="$1"
  local default="$2"
  local context="${3:-}"  # Default to empty string if not provided
  local value
  value=$(getJsonValue "$key")
  if [[ -z "$value" || "$value" == "null" ]]; then
    value="$default"
    # Ensure the context and dash are only included if context is set
    if [[ -n "$context" ]]; then
      PostDebug "$context - Using default value: $value"
    else
      PostDebug "Using default value: $value"
    fi
  fi
  echo "$value"
    ...
    case "$1" in
      ...
      -v|--verbose)  
        if [[ -n "$2" && "$2" =~ ^(true|false)$ ]]; then
          LOCAL_VERBOSE_INFO="$2"
          shift # Consume the next argument
        else
          LOCAL_VERBOSE_INFO=true # Default to true if no argument is provided
        fi
        ;;
...
# Export final return values
export RETURN_CODE VALID_STATE
PostInfo "Completed Pre-Commit Hook Processing"
exit "$VALID_STATE"
